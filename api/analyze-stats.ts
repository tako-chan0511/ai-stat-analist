...
async function fetchSingleDataSet(appId: string, statsDataId: string, filters: any): Promise<EStatDataValue[]> {
  ...
  if (eStatData.GET_STATS_DATA.RESULT.STATUS !== 0) {
    if (eStatData.GET_STATS_DATA.RESULT.ERROR_MSG.includes('該当データはありません')) return [];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // ★★★ ここを 'userApiKey' から 'geminiApiKey' に修正 ★★★
  const { statsDataId, analyses, question: initialQuestion, geminiApiKey } = req.body;
  const eStatAppId = process.env.ESTAT_APP_ID;

  if (!statsDataId || !analyses?.length || !eStatAppId || !geminiApiKey) {
    if (!geminiApiKey) {
      return res.status(400).json({ error: 'Gemini APIキーが設定されていません。右上の歯車アイコンから設定してください。' });
    }
  ...
  try {
    const datasetsRaw = await Promise.all(
      (analyses as AnalysisTarget[]).map(analysis => fetchSingleDataSet(eStatAppId, statsDataId, analysis.filters))
    );
    const processedDatasets = datasetsRaw.map((values, i) => {
      const yearlyData = values.reduce((acc: { [key: string]: number }, v: EStatDataValue) => {
        const year = v['@time'].substring(0, 4);
        const value = parseFloat(v['$']);
        if (!isNaN(value)) acc[year] = value;
        return acc;
      }, {});
      return { name: Object.values(analyses[i].filterNames).join(' - '), data: yearlyData };
    });
